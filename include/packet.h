...
struct PacketHeader
{
    ...
    {
        size_t written = 0;
        written += ::emit(type, out);
        written += ::emit(payloadLength, out);
...
public:
    ...
    size_t serializeImpl(OutT &out)
    {
        ...
        size_t payloadLength = getPayloadSize(name.size(), extra.size());
...
template <typename ItrT>
MsgType peekType(ItrT begin, ItrT end)
{
    MsgType type = MsgType::EMPTY;
